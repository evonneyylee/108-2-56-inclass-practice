...
##cut：連續轉類別  ## switch常跟cut一起用  ##把數值資料變成類別資料(levels)
...
```{r}
whatNeedToDo_butler_mom <- function(nowtime=now()){   ##架構成function
  library(lubridate)
  library(glue)

  cutpoints <-
    c(
      glue("{today()} 06:00:00"),  ##06
      glue("{today()} 09:00:00"),  ##09 
      glue("{today()} 17:00:00"),  ##17
      glue("{today()} 19:00:00"),  ##19
      glue("{today()} 23:30:00"),  ##23
      glue("{today()+days(1)} 01:00:00")
    )
  cutpoints %>%
    ymd_hms(tz="Asia/Taipei") -> cutpoints
