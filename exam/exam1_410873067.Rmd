...
sum300 <- sum(as.numeric(as.factor(receipts300[1:300])))
sum300
```
...
```{r ans21}
poem_song [order(poem_songOrder)]
poem_song
```
...
polandTime <- ymd_hms("2020-4-25 3:50:00",tz="Europe/Warsaw")
with_tz(polandTime, tzone="Asia/Taipei")
post$comments$gender[[26]] <- "男"
post$comments$DateTime[[26]] <- "4月25日 9:50"
post$comments$content[[26]] <- "心不在課堂上，在哪都做得到遠距。"
post$comments$hearts[[26]] <- as.numeric("0")
