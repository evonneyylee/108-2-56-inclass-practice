...
selectedMarkets[c(1,5,10,14)] <-T # generic replacement
print(selectedMarkets)
```
...
```
```{r}
str(get1,max.level = 1) #兩個中括號，「拿出來」（list of2下面我們要的值）
```
```{r}
str(get2, max.level = 1)  #兩個中括號，「只留下」（只有我們最外層的，不是我們要的
```
